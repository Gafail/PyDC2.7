...
class PyDC:
    ...
    def say(self, message):
        self.sock.send('<' + self.nick + '> ' + self.dc_escape(message) + '|')
        
    def pm(self, user, message):
        self.sock.send('$To: ' + user + ' From: ' + self.nick + ' $<' + self.nick + '> ' + self.dc_escape(message) + '|')
    ...
    def handle (self, data):
        ...
        if cmd == '$Lock':
            key = self.locktokey(rem)
            self.sock.send(
                '$Supports NoGetINFO UserCommand UserIP2|'+
                '$Key '+key+'|'+
                '$ValidateNick '+self.nick+'|'
            )
            return
        ...
        if cmd == '$Hello':
            if (rem == self.nick):                
                ...
                self.sock.send('$GetNickList|')
                self.sock.send('$MyINFO '+ self.getmyinfo()+'|')
            else:
                if (rem not in self.users.keys()):
                    self.users[rem] = ''
                    self.onUserJoin(rem)
            return
        ...
        if cmd == '$GetPass':
            self.sock.send('$MyPass '+self.password+'|')
            return
